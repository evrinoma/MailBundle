...
use Evrinoma\MailBundle\Factory\Mail\Factory as MailFactory;
use Evrinoma\MailBundle\Mediator\QueryMediatorInterface;
use Evrinoma\MailBundle\Repository\Mail\MailCommandRepositoryInterface;
...
class EvrinomaMailExtension extends Extension
{
    use HelperTrait;

    public const ENTITY = 'Evrinoma\MailBundle\Entity';
    public const MODEL = 'Evrinoma\MailBundle\Model';
    public const ENTITY_FACTORY_MAIL = MailFactory::class;
    public const ENTITY_BASE_MAIL = BaseMail::class;
    public const DTO_BASE_MAIL = MailApiDto::class;
    ...
    public function load(array $configs, ContainerBuilder $container)
    {
        ...
        if (self::ENTITY_FACTORY_MAIL !== $config['factory']) {
        ...
        $this->wireForm($container, $config['dto'], 'mail', 'mail');
    ...
    private function wireConstraintTag(ContainerBuilder $container): void
    {
        foreach ($container->getDefinitions() as $key => $definition) {
            switch (true) {
                case str_contains($key, MailPass::MAIL_CONSTRAINT)   :
                    $definition->addTag(MailPass::MAIL_CONSTRAINT);
                    break;
                default:
            }
        }
    ...
    private function wireRepository(ContainerBuilder $container, Reference $registry, string $madiator, string $class, string $driver): void
    {
        ...
        $container->addAliases([MailCommandRepositoryInterface::class => 'evrinoma.'.$this->getAlias().'.repository']);
        $container->addAliases([MailQueryRepositoryInterface::class => 'evrinoma.'.$this->getAlias().'.repository']);
    }
    ...
    {
        return EvrinomaMailBundle::BUNDLE;
    }
}
